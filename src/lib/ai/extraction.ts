...
const EXTRACTION_SYSTEM_PROMPT =
  "You extract structured data from a photo or scan of an invoice, bill or receipt. " +
  "Read ONLY what is printed on the document — never guess or infer a number that is not " +
  "visibly present. Respond with ONLY a single JSON object, no prose, no markdown fences, " +
  'matching exactly: {"vendorName": string|null, "documentDate": string|null (YYYY-MM-DD), ' +
  '"amount": number|null (the total, including tax), "taxAmount": number|null, ' +
  '"currency": string|null (ISO 4217 code if visible), "description": string|null (one line, ' +
  'what was purchased), "confidence": "high"|"medium"|"low"}. If a field is not legible or not ' +
  "present, use null for it rather than guessing.";
...
export async function extractDocument(params: {
  ...
  const document = await prisma.document.create({
    data: {
      ...
      // TODO(Phase 7+/production): upload imageBase64 to object storage
      // and store the real key here instead.
      storageKey: `unstored:${params.fileName}`,
      hash,
      mimeType: params.mimeType,
      sizeBytes: Math.ceil((params.imageBase64.length * 3) / 4),
      status: "PROCESSING",
