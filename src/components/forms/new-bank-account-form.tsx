...
export function NewBankAccountForm() {
  ...
  return (
    <form onSubmit={submit} className="grid grid-cols-4 gap-3 rounded-lg border border-border bg-card p-4">
      <input required placeholder="Account name (e.g. Emirates NBD Current)" value={name} onChange={(e) => setName(e.target.value)} className="col-span-2 rounded-md border border-border bg-background px-3 py-2 text-sm" />
      ...
      <button type="submit" disabled={loading} className="col-span-4 rounded-md bg-primary px-3 py-2 text-sm font-medium text-primary-foreground disabled:opacity-50">
